...
afc.beginTimeCheck = function(msg)
{
	...
	if(!msg) msg = '';
	afc.log(msg + ' Start time ==>			' + afc.startTime + ' --------------------------------------------------');
};

afc.ellapseCheck = function(msg)
{
	if(afc.startTime==0) return;
	
	if(!msg) msg = '';
	
	afc.log(msg + ' Ellapsed time ==>		' + (Date.now() - afc.oldTime));
...
afc.endTimeCheck = function(msg)
{
	...
	afc.log(msg + ' End time ==> 			' + afc.oldTime + ' -------------------------------------');
	afc.log(msg + ' Total Ellapsed time ==>	' + (afc.oldTime - afc.startTime) + ' -------------------------------------');
	afc.startTime = 0;
	afc.oldTime = 0;
};
...
afc.loadSync = function(trgEle, url, callback, searchValue, newValue, isAsync) 
{
...
afc.loadCss = function(url, attrObj)
{
	if(!afc.cssMap[url])
	{
		...
		$('<link rel="stylesheet" href="' + url + '"' + strAttr + '/>').appendTo('head');
		afc.cssMap[url] = true;
	}
};
...
afc.makeMeta = function()
{
	...
	if(PROJECT_OPTION.general.autoScale)
	{
		...
		else
		{
			...
			//if(scale>1.0) scale = 1.0;	왜 확대를 막았는지 좀 더 명확히 이유 파악한 후 주석 풀기
	...
	{ 
		...
		meta += !docWidth ? 'device-width' : docWidth;
...
afc.deviceCheck = function()
{
	...
	if(afc.isMobile)
	{
		...
		//###########################################################################################################
		// 아래 비교로 명확히 구별되는지 차후 검증해 볼 것! 예외 상황이 있을 경우 window.devicePixelRatio 값도 비교해 보기
		//###########################################################################################################

		afc.isTablet = (chkWidth>afc.PHONE_DOC_WIDTH);	//640
		afc.isPhone = !afc.isTablet;
	}
	
	
	//시뮬레이터인데 브릿지 셋팅이 되어 있으면 모바일 처럼 작동하기위해 
	//스크롤바를 숨김
	//if( afc.isMobile || (afc.isSimulator && afc.isHybrid) )
	if( afc.isSimulator && afc.isHybrid )
	{
